...
const PeopleList = ({ onSelect }) => {
  ...
  return (
    <div className="people-list">
      {peoples.map((people) => (
        <div key={people._id} onClick={() => onSelect && onSelect(people)} className="people-item d-flex align-items-center">
          <img src={people.image} alt={people.name} className="people-avatar" />
          <div className="people-info">
            <div className="people-header d-flex justify-content-between">
              <span className="people-name">{people.name}</span>
              <span className="people-time">{formatDateTime(people.lastMessageTime || 0)}</span>
            </div>
